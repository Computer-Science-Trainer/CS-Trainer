...
from database import redis_client, LOGIN_ATTEMPT_LIMIT, BLOCK_TIME_MINUTES
...
def login(data: LoginRequest):
    block_key = f"login_block:{data.email}"
    if redis_client.exists(block_key):
        raise HTTPException(
            status_code=429,
            detail={"code": "too_many_attempts", "message": "Try again later"}
        )

    # Счетчик попыток
    attempt_key = f"login_attempts:{data.email}"
    attempts = redis_client.incr(attempt_key)

    if redis_client and attempts >= LOGIN_ATTEMPT_LIMIT:
        redis_client.setex(block_key, BLOCK_TIME_MINUTES * 60, "1")
        raise HTTPException(
            status_code=429,
            detail={"code": "too_many_attempts", "message": "Account temporarily locked"}
        )
    ...
    #     {'sub': str(data.email), 'user_id': user['id']})
    if user['email'] in ADMIN_EMAILS:
        access_token = create_access_token({"sub": str(data.email), "user_id": user["id"], "role": "admin"})
    else:
        access_token = create_access_token({"sub": str(data.email), "user_id": user["id"], "role": "user"})
    print(access_token)
...
def register(data: RegisterRequest):
    ...
    code = generate_verification_code()
    if save_user(data.email, hash_password(data.password),
...
def change_password(data: ChangePasswordRequest):
    ...
    if data.code != user['verification_code']:
        raise HTTPException(
            status_code=400, detail={
                'code': ErrorCodes.INVALID_VERIFICATION_CODE})
    if change_db_users(data.email, ('password', hash_password(data.password)),
                       ('verified', 1)) != 'success':
        raise HTTPException(
            status_code=500, detail={
                'code': ErrorCodes.SAVING_FAILED})
