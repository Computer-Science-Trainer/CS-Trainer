...
from services.tests_service import start_test, get_test_questions, submit_test
...
def start_test_route(body: TestStartIn, authorization: str = Header(None, alias="Authorization")):
...
def get_test_questions_route(test_id: int, authorization: str = Header(None, alias="Authorization")):
    user_id = authorize(authorization)
    return get_test_questions(user_id, test_id)

@router.post("/{test_id}/submit", response_model=TestResult)
def submit_test_route(test_id: int, body: TestSubmissionIn, authorization: str = Header(None, alias="Authorization")):
    user_id = authorize(authorization)
